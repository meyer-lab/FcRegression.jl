...
function figureS3()
    ...
    c0, ccdf0 = FcRegression.runRegMCMC(df; 
        murine = true, Kav = Kav, mcmc_iter = 1000, link = FcRegression.exponential, cellTypes = ["ncMO", "cMO", "NKs", "Neu", "EO"])

    c1, ccdf1 = FcRegression.runRegMCMC(df; 
        murine = true, Kav = Kav, mcmc_iter = 1000, link = FcRegression.tanh, cellTypes = ["ncMO", "cMO", "NKs", "Neu", "EO"])
    ...
    draw(PDF("output/figureS3.pdf", 5inch, 3inch), pl)
end