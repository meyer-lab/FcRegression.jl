function dist_violin_plot(df, dist_list; title = "", x_name = "Receptor")
    ...
    odf = DataFrame([names(df)[i] => rand(dist_list[i], 600) for i = 1:size(df)[2] if dist_list[i].μ > 0])
...
function plot_MCMC_affinity(c = runMCMC())
    ...
    for igg in Kav_priors[!, "IgG"]
        ...
        append!(pl_igg, [dist_violin_plot(posts, priors; title = "$igg Affinities Distributions")])
    end
