...
function figureS2()
    ...
    pl1 = plot(df1, x = "Cells", y = "Abundance", ymin = "ymin", ymax = "ymax", color = "Receptor", 
            Geom.bar(position = :dodge), Geom.errorbar, Stat.dodge, Scale.x_discrete, Scale.y_log10,
            style(bar_spacing=1.5mm), Guide.title("Murine immune cell FcR expression"))
    ...
    pl2 = plot(df2, x = "Cells", y = "Abundance", ymin = "ymin", ymax = "ymax", color = "Receptor", 
            Geom.bar(position = :dodge), Geom.errorbar, Stat.dodge, Scale.x_discrete, Scale.y_log10,
            style(bar_spacing=1.5mm), Guide.title("Human immune cell FcR expression"))
    draw(SVG("figureS2.svg", 1000px, 400px), plotGrid((1, 2), [pl1, pl2]))
end
