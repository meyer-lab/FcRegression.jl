...
function plotActualvFit(odf, dataType)
    ...
    pl = plot(
        odf, x = :Y, y = :Fitted, Geom.point,
        color = :Condition, shape = :Background,
        Guide.colorkey(pos=[0.05w,0.28h]),
        Scale.y_continuous(minvalue=0.0, maxvalue=1.0),
        slope=[slope], intercept=[intercept], Geom.abline(color="red"),
        Guide.xlabel("Actual effect"), Guide.ylabel("Fitted effect"),
...
function plotActualvPredict(odf, dataType)
    ...
    pl = plot(
        odf, x = :Y, y = :LOOPredict, Geom.point,
        color = :Condition, shape = :Background,
        Guide.colorkey(pos=[0.05w,0.5h]),
        slope=[slope], intercept=[intercept], Geom.abline(color="red"),
        Guide.xlabel("Actual effect"), Guide.ylabel("LOO predicted effect"),
...
function plotCellTypeEffects(wdf, dataType)
    ...
    pl = plot(
        wdf, x = :Condition, y = :Weight, color = :Component,
        Guide.colorkey(pos=[0.05w,-0.28h]),
...
function plotDepletionSynergy(IgGXidx::Int64, IgGYidx::Int64, weights::Vector, L0, f;
...
function figureW(dataType; L0 = 1e-9, f = 4)
    ...
    p4 = plotDepletionSynergy(2, 3, fit_w, 1e-9, 4;
        c1q=(:C1q in unique(wdf.Component)))
