...
function calcSynergy(IgGXidx::Int64, IgGYidx::Int64, L0, f, FcExpr, Kav; murine, fit = nothing, ActI = nothing, c1q = false, nPoints = 100)
...
function synergyGrid(L0, f, FcExpr, Kav; murine, fit = nothing, ActI = nothing, c1q = false)
    M = zeros(size(Kav)[1], size(Kav)[1])
    nPoints = 100

    for i = 1:size(Kav)[1]
        for j = 1:(i - 1)
            D1, D2, additive, output = calcSynergy(i, j, L0, f, FcExpr, Kav; murine = murine, fit = fit, ActI = ActI, c1q = c1q, nPoints = nPoints)
